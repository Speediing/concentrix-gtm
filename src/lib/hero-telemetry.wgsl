...
fn concentrixMark(p: vec2f) -> f32 {
  let q = rotate(p, -0.12);
  let outer = abs(sdEllipse(q, vec2f(0.22, 0.14))) - 0.009;
  let middle = abs(sdEllipse(q - vec2f(0.025, 0.0), vec2f(0.145, 0.092))) - 0.008;
  let inner = abs(sdEllipse(q - vec2f(0.05, 0.0), vec2f(0.072, 0.046))) - 0.007;
...
fn fs_main(@location(0) uv: vec2f) -> @location(0) vec4f {
  ...
  let dMark = concentrixMark(markP);
  ...
  let teal = vec3f(0.078431, 0.549020, 0.568627);
  let paper = vec3f(0.956863, 0.925490, 0.862745);
  let leftClear = smoothstep(0.34, 0.62, uv.x);
  var a = fill * 0.08 + line * 0.28 + glow * 0.08 + traces * 0.32 + spark * 0.06;
  a *= 0.55 * leftClear;
  a = clamp(a, 0.0, 0.34);
  let col = mix(teal, paper, spark * 0.45 + line * 0.05);
